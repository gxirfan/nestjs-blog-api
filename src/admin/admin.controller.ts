...
import { ITagResponse } from 'src/forum/tags/interfaces/tag.response.interface';
...
import { ITopicResponse } from 'src/forum/topics/interfaces/topic-response.interface';
...
export class AdminController {
    ...
    async updateUser(@Param('id') id: string, @Body() user: UpdateUserByAdminDto): Promise<IUserResponse> {
        return await this.userService.updateUserByAdmin(id, user);
    }

    @Patch('update-password/:id')
    @ResponseMessage('User password updated successfully.')
    async updatePassword(@Param('id') id: string, @Body() newPassword: string): Promise<IUserResponse> {
        return await this.userService.updatePasswordByAdmin(id, newPassword);
    }

    @Delete('delete-user/:id')
    @ResponseMessage('User deleted successfully.')
    async deleteUser(@Param('id') id: string): Promise<IUserResponse> {
    ...
    async tagUpdateOneById(@Param('id') id: string, @Body() tag: UpdateTagDto): Promise<ITagResponse> {
    ...
    async tagDeleteOneById(@Param('id') id: string): Promise<ITagResponse> {
    ...
    async findAllTopics(): Promise<ITopicResponse[]> {
        return TopicMapper.toResponseDto(await this.topicService.findAll());
    }

    @Get('get-topic/:id')
    @ResponseMessage('Topic fetched successfully.')
    async topicFindOneById(@Param('id') id: string): Promise<ITopicResponse> {
    ...
    async topicUpdateOneById(@Param('id') id: string, @Body() topic: UpdateTopicDto): Promise<ITopicResponse> {
    ...
    async topicDeleteOneById(@Param('id') id: string): Promise<ITopicResponse> {
